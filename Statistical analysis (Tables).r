...
########## RULE COMPLIANCE: TRAFFIC LIGHTS TASK (A-B-A design based on Gaechter et al 2022) ######
...
a$stuck<-ifelse(a$comply2==a$comply3, 1, 0)
...
model3<-glm(stuck ~  female + age + comply1, data=subset(a, a$following==1), family='binomial')

### output HTML file: this is Table S2 #######
library('stargazer')
stargazer(model0, model1,model2, model3,type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, star.cutoffs=c(0.05,0.01,0.001), single.row=TRUE,
 report=('vcsp'),
	out="Table S2.html")
...
# paired t-tests to check if adjustments were significantly different from 0
t.test(a$d2s, a$d1s, paired=TRUE)
t.test(a$d2f, a$d1f, paired=TRUE)


### what portion of participants had a different initial decision from the social information 
### (and hence could meaningfully adjust their donation based on social information)?
...
# means and 95% CIs for reductions and increases in donations
p_s<-mean(a$deltaSelfish, na.rm=T)
se_s<-sd(a$deltaSelfish, na.rm=T) /sqrt(length(na.omit(a$deltaSelfish)))
ci_s_lower<- p_s - 1.96 * se_s
ci_s_upper<- p_s + 1.96 * se_s

p_f<-mean(a$deltaFair, na.rm=T)
se_f<-sd(a$deltaFair, na.rm=T) /sqrt(length(na.omit(a$deltaFair)))
ci_f_lower<- p_f - 1.96 * se_f
ci_f_upper<- p_f + 1.96 * se_f
...
plot(rMat$E1/rMat$T, rMat$X/rMat$T, xlim=c(0,3), ylim=c(0,3))
arrows(-10,-10, 200, 200, code=0)
arrows(1,-1, 1, 4, code=0)
arrows(-1,1, 4, 1, code=0)
...
### check: does the within-participant variance in adjustments vary with age?
meanVariance<-c()
meanUpdateInExpectedRange<-c()
ages<-c();

par(mfrow=c(1,2), xaxs='r', yaxs='r')

for (i in 10:20){
	ee<-subset(rMat, rMat$age==i)
	vars<-c()
	for (ind in unique(ee$participantNr)){
		ff<-subset(ee, ee$participantNr==ind)
		vars<-c(vars, var(ff$s))
	}
	
	cntWithinExpRange<-0;
	for (k in 1:nrow(ee)){
		if (ee$s[k]>=0) cntWithinExpRange<-cntWithinExpRange+1;
	}
	meanVariance<-c(meanVariance, mean(vars))
	ages<-c(ages, i)
	meanUpdateInExpectedRange<-c(meanUpdateInExpectedRange, cntWithinExpRange/nrow(ee))
}
plot(ages, meanVariance)

plot(ages, meanUpdateInExpectedRange, ylab='P in expected range', ylim=c(0.8, 1))
...
for (i in 1:nrow(b)){
	for (j in 1:5){
		...
		dev<-(E1-correct[j])/correct[j] # relative deviation in E1
		...
		r<-c(b$participantNr[i], b$age[i], b$female[i], j, s, dev)
		d<-rbind(d, r)
		
	}
}
d<-data.frame(d)
names(d)<-c('participantNr', 'age', 'female', 'period', 'adjustment', 'deviation')
...
### does adding the initial error in a round make a difference for the main result?
model4<-lmer(adjustment ~ age + female + deviation + (1 | participantNr), data=d)
summary(model4)
# no it does not
...
# first make the binary variable an ordered variable

a$following2<-ordered(a$following)

# define the model, including age as a regressor
HS.model <- 
'sensit =~ following2 + S_DG + BEAST_S
sensit ~ age'
#fit the model
fit <- cfa(HS.model, data=a)#, std.lv=TRUE)
# inspect the output
summary(fit, fit.measures=TRUE)#, standardized=TRUE)
standardizedsolution(fit)


# write a table with only the valid data (e.g. for plotting)

b<-subset(a,!is.na(a$following))
b<-subset(b,!is.na(b$S_DG))
b<-subset(b,!is.na(b$BEAST_S))
b$fa1<-predict(fit)

write.table(b, file='data_with_factors3.txt', sep='\t')
